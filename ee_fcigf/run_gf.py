...
import math
import sys
from pyscf.ee_fcigf import compute_gf
...
#    e_fc += 2 * np.einsum('ppqq', v2e[:ncore,:ncore,:ncore,:ncore]) - np.einsum('pqpq', v2e[:ncore,:ncore,:ncore,:ncore])
    ...
    if(dos_only):
        for omega_val in omega_array:
            ...
            #gf_real_addition[0,0], gf_imag_addition[0,0] = gf_addition(romega, iomega, delta, e_cas, 0, 0, mc, h1eff, v2e, tol_gf, maxiter)
            gf_real_removal[0,0], gf_imag_removal[0,0] = gf_removal(romega, iomega, delta, e_cas, 0, 0, mc, h1eff, v2e, tol_gf, maxiter)
        
            gf_real_trace.append(np.trace(gf_real_removal))
            gf_imag_trace.append(np.trace(gf_imag_removal))
            dos = -np.trace(gf_imag_removal)
            density_of_states.append(dos)
            print "Optical spectrum :             ", dos
